...
            function SavePassport(result_id,form_id,url) {
                jQuery.ajax({
                    url:     url, 
                    type:     "POST", 
                    dataType: "html", 
...
<body>
    <div class="container" id="passport_view">
        ...
        <div>       
            <form class="form-passport" id="form_passport_set">
            <label for="login" class="cols-sm-2 control-label"><?php echo LANG_PASSPORT_SET_LOGIN ?></label>
            <input id="login" name="login" class="form-control" type="text" value="<?php echo $passport['login'];?>" required></p>
            ...
            <p><input type="button" class="btn btn-lg btn-primary" type="submit" value="<?php echo LANG_PASSPORT_SET_SAVE; ?>" onclick="SavePassport('save_result', 'form_passport_set', '<?php echo \unit_Html::href($this->module->name . '/save'); ?>')"></p>
